...
def build():
  ...
  try {
    ...
    add(story, s["normal"], [
        ...
        "The authentication system also integrated into the frontend through JWTs. After login, the frontend stored the user and token information through AuthContext.jsx, and later requests sent the JWT in the Authorization: Bearer header. This mattered because pages such as Home, Chores, Expenses, Calendar, and Profile relied on the backend knowing which user was making the request. I also pushed for guarded routes because I wanted to make sure users could not manually type protected URLs and access private roommate data without being logged in. Emma implemented the final ProtectedRoutes and LoggedInProtectedRoutes components, but the idea came from trying to make authentication a real access-control boundary rather than just a login form.",
    ])
    story.append(PageBreak())
    ...
    add(story, s["normal"], [
        "If I had more time, I would first improve our handling of group state. Conditional rendering worked well for the Home page, but over time Chores, Expenses, and Calendar also became dependent on whether a user belonged to a group. A GroupContext or GroupProtectedRoute would have made that dependency clearer and would have reduced repeated fetches and repeated conditional logic.",
        ...
        "On the product side, I would also add notifications for upcoming chores, better expense settlement history, and a clearer onboarding flow for join codes. Those features would have made the app feel more complete for real roommate use, but the testing improvements would have mattered first because they would have made it safer to keep adding features without accidentally breaking existing flows.",
    ])
