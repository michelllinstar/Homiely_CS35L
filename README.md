...
└── e2e/
    └── homily.spec.js      # End-to-end test suite
```
...
`DATABASE_URL` — PostgreSQL connection URI. Defaults to `postgresql://user:password@db/homily` (the Docker internal hostname). Must be overridden for local (non-Docker) runs.
